...
func (r Result) FactBundle(playerMessage string) FactBundle {
	...
	for _, outcome := range r.Outcomes {
		hasFailure, outcomeClarification := false, false
		for _, fact := range outcome.Result.VisibleFacts {
			...
			outcomeClarification = outcomeClarification || fact.Kind == game.FactClarification
		}
		...
		if (outcome.Result.Status == game.ActionClarification || outcome.Result.NeedsClarification) && !outcomeClarification {
	...
	if strings.TrimSpace(r.ClarificationQuestion) != "" && !hasClarification {
		bundle.Facts = append(bundle.Facts, game.Fact{Kind: game.FactClarification, Subject: "turn", Value: "needs_clarification", Text: r.ClarificationQuestion, Required: true})
	}
